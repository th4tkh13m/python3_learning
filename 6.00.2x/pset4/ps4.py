...
def r_squared(y, estimated):
    ...
    """
    mean = sum(y) / len(y)
    numerator = ((np.array(y) - np.array(estimated))**2).sum()
    denominator = ((np.array(y) - mean)**2).sum()
...
def evaluate_models_on_training(x, y, models):
    ...
    """
    for model in models:
        est_yVals = np.polyval(model, x)
        r_2 = r_squared(y, est_yVals)
        print(r_2)
        pylab.title(f"Regression Model's Degree: {len(model) - 1}\nR^2 = {r_2}")
        pylab.plot(x, y, 'bo')
        pylab.plot(x, est_yVals, 'r', label = 'Regression')
        pylab.xlabel("Year")
        pylab.ylabel("Temp")
...
# Graph and regress the temperature of a specific day (1st October) yearly
pylab.figure('daily')
y = []
x = INTERVAL_1
...
# Graph and regress the mean temperature yearly
pylab.figure('yearly')
x1 = INTERVAL_1
x2 = INTERVAL_2
y = []
