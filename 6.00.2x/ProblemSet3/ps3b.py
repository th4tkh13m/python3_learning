...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    # TODO
    yVals = np.array(300*[0])
    for trial in range(numTrials): 
        viruses = [SimpleVirus(maxBirthProb, clearProb) for x in range(numViruses)]
        khiem = Patient(viruses, maxPop)
        for time in range(300):
            yVals[time] = yVals[time] + khiem.update()
    yVals = yVals/numTrials
    pylab.plot(yVals, label = "SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if random.random() > self.getMaxBirthProb() * (1 - popDensity):
            raise NoChildException
        offspring_resistance = self.getResistances().copy()
        for drug in self.getResistances():
            if random.random() <= self.getMutProb():
                if self.isResistantTo(drug):
                    offspring_resistance[drug] = False
                else:
                    offspring_resistance[drug] = True
        return ResistantVirus(self.getMaxBirthProb(),\
                self.getClearProb(), offspring_resistance, self.getMutProb())
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.getViruses():
            if all([virus.isResistantTo(drug) for drug in drugResist]):
                resistance_pop = resistance_pop + 1
        return resistance_pop
    ...
    def update(self):
        ...
        integer)
        """
        virus_copy = self.getViruses().copy()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    """
    yVals, yVals_withDrug = [0 for x in range(300)], [0 for x in range(300)]
    for trial in range(numTrials):
        viruses = [ResistantVirus(maxBirthProb, clearProb,\
                    resistances, mutProb) for count in range(numViruses)]
        khiem = TreatedPatient(viruses, maxPop)
        for step in range(300):
            if step == 150:
                khiem.addPrescription('guttagonol')
            new_pop = khiem.update()
            yVals[step] = yVals[step] + new_pop
            resistance_pop = khiem.getResistPop(['guttagonol'])
            yVals_withDrug[step] = yVals_withDrug[step] + resistance_pop
    y1 = [x/numTrials for x in yVals]
    y2 = [x/numTrials for x in yVals_withDrug]
    pylab.plot(y1, label = "ResistanceVirus")
    pylab.plot(y2, label = "Drug resisted")
