...
def buildCityGraph(graphType):
    ...
    g.addEdge(WeightEdge(g.getNode('Providence'), g.getNode('New York'), 15))
    g.addEdge(WeightEdge(g.getNode('New York'), g.getNode('Chicago'), 30))
    g.addEdge(WeightEdge(g.getNode('Chicago'), g.getNode('Phoenix'), 10))
...
#weighted DFS
def WeightedDFS(graph, start, end, path, shortest, weight = 0, toPrint = False):
    """Assumes graph is a Digraph; start and end are nodes;
          path and shortest are lists of nodes
       Returns a shortest path from start to end in graph"""
    path = path + [start] 
    ...
    for node in graph.childrenOf(start):
        if node not in path: #avoid cycles
            if shortest == None or (len(path) > 1 and len(shortest) > 1 and 
            sum([graph.getEdgeWeight(path[i], path[i + 1]) for i in range(len(path)-1)]) < sum([graph.getEdgeWeight(shortest[i], shortest[i + 1]) for i in range(len(shortest)-1)])):
                newPath = WeightedDFS(graph, node, end, path, shortest,
                              toPrint)
                if newPath != None:
                    shortest = newPath
        elif toPrint:
...
def mod_shortestPath(graph, start, end):
    """Assumes graph is a Digraph; start and end are nodes
       Returns a shortest path from start to end in graph"""
    return WeightedDFS(graph, start, end, [], None)
...
def testSP(source, destination):
    ...
    sp = mod_shortestPath(g, g.getNode(source), g.getNode(destination))
    if sp != None:
        print('Shortest path from', source, 'to',
              destination, 'is', printPath(sp))
    else:
        print('There is no path from', source, 'to', destination)
    print(sum([x.getWeight() for x in sp]))
